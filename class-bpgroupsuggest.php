...
if ( ! class_exists( 'BPGroupSuggest' ) ) :
	...
	class BPGroupSuggest {
		...
		private function __construct() {
			...
			include_once 'groups/suggested-groups.php';
		}
		...
		public static function get_suggestions_groups_id( $limit = null, $user_id = null ) {
			...
			if ( ! empty( $possible_groups ) ) {
				...
				if ( $limit ) {
					$groups_sug = array_slice( $possible_groups, 0, $limit );
				} else {
					$groups_sug = $possible_groups;
				}
				return $groups_sug;
			}
			return false;
		...
		public static function suggestions_list( $limit = null, $mikos = null, $show_join = null, $user_id = null, $args = '' ) {
			...
			$groups_sug = self::get_suggestions_groups_id( null, $user_id );

			if ( ! empty( $groups_sug ) ) :
				if ( $limit ) {
					$groups_sug = array_slice( $groups_sug, 0, $limit );
				}
				?>
				<ul id="<?php echo $ul_id; ?>" class="<?php echo $ul_class; ?>">
				<?php
				foreach ( $groups_sug as $group_id ) :
					$group_sug = groups_get_group( array( 'group_id' => $group_id ) );
					?>
			<li class="<?php echo $li_class; ?>">
			<div class="item-avatar"><a href = "<?php bp_group_permalink( $group_sug ); ?>"	title = "<?php bp_group_name( $group_sug ); ?>">
			    <?php
			    echo bp_core_fetch_avatar(array( 'type'    => 'thumb', 'width'  => 50, 'height'  => 50, 'object'  => 'group', 'item_id' => $group_id, ));?>
			    </a>
			</div>
			<div class="item">
		...
		public function get_suggestions_list( $limit = null, $mikos = null, $show_join = null, $user_id = null ) {
			...
			if ( ! empty( $groups_sug ) ) :
				...
				foreach ( $groups_sug as $group_id ) :
					...
					. '		<img src="' . plugins_url()  . '/bp-group-suggestions/images/icon-tool.png" alt="' . __( 'Hide this suggestion', 'bp-groups-suggestions' ) . '"></a>'
					. '</div>';
